...
class StaticPlacer
{
    ...
    float get_steplen()
    {
        ...
        printf("coord_dist: %f grad_dist: %f\n", coord_dist, grad_dist);
    ...
    void update_chains()
    {
        ...
        float alpha = std::min<float>(std::pow(1.004f, iter) - 1, 1.0f);
        for (int i = 0; i < int(macros.size()); i++) {
            ...
            for (int c : macro.conc_cells) {
                ...
                mc.ref_pos = mc.ref_pos * (1 - alpha) + (mc.pos + RealPair(cc.chunk_dx, cc.chunk_dy)) * alpha;
            }
        }
    }
    ...
    void step()
    {
        ...
        nesterov_a = a_next;
        // update_chains(); TODO: find out why this makes attosoc diverge
        update_gradients(true);
